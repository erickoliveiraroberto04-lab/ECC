...
- `npx skills add anthropics/skills -s academy-guide -s discernment-nudge -s brand-guidelines -s doc-coauthoring -s internal-comms -s mcp-builder -s slack-gif-creator -s theme-factory -s web-artifacts-builder -s webapp-testing` - Install the rest of Anthropic's official example skills not already bundled with Claude Code (Claude Academy course matching, post-answer discernment nudges, Anthropic brand styling, doc co-authoring, internal comms templates, MCP server builder, Slack GIF creation, artifact theming, complex React/Tailwind/shadcn artifacts, and Playwright-based local webapp testing)
